...
def command_url(cmd, args=None):
    return 'subl:' + html_format_command(cmd, args)
...
class Window():
    ...
    def __repr__(self):
        return 'Window(%r)' % self.window_id
...
class TextChange():
    ...
    def __init__(self, pa, pb, len_utf16, len_utf8, s):
        self.a = pa
        self.b = pb
...
class ImageSheet(Sheet):
    def __repr__(self):
        return 'ImageSheet(%r)' % self.sheet_id
...
class Buffer:
    ...
    def __repr__(self):
        return 'Buffer(%r)' % self.buffer_id
...
class CompletionList:
    ...
    def __repr__(self):
        return 'CompletionList(completions=%s, flags=%s)' % (
            repr(self.completions),
            repr(self.flags)
        )
...
class ListInputItem:
    ...
    def __repr__(self):
        return 'ListInputItem(%r, %s, details=%r, annotation=%r, kind=%r)' % (
            self.text,
            self.value,
            self.details,
            self.annotation,
...
class SymbolRegion:
    ...
    def __repr__(self):
        return 'SymbolRegion(%r, %r, syntax=%r, type=%r, kind=%r)' % (
            self.name,
            self.region,
            self.syntax,
            self.type,
...
class SymbolLocation:
    ...
    def __repr__(self):
        return 'SymbolLocation(%r, %r, row=%r, col=%r, syntax=%r, type=%r, kind=%r)' % (
            self.path,
            self.display_name,
            self.row,
            self.col,
